...
class VerticalCarousel extends React.Component {
  ...
  getPresentableSlides() {
    ...
    this.props.parentCallback(presentableSlides[2].key);
    if (this.state.currentIndex !== presentableSlides[2].key) {
      this.setState({currentIndex: presentableSlides[2].key, currentContent: presentableSlides[2].content})
    }
        ...
        <Wrapper>
          {this.getPresentableSlides().map((slide, presentableIndex) => (
            <Slide
              ...
              activeContent={this.state.currentContent}
            />
          ))} 
        </Wrapper>
      </React.Fragment>
